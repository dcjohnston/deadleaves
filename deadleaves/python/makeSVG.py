...
def main():
    ...
    wd = os.getcwd()
    filenames, alpha, size, outDir, backgroundColor = parseInput()

    N = getN(alpha)

    emojis = randEmojis(N=N, filenames=filenames)

    scaledEmojis = scaleEmojis(
        emojis, alpha=0.1, xmin=alpha / 10, xLim=size[0], yLim=size[1])

    fig = sg.SVGFigure(str(size[0]) + "px", str(size[1]) + "px")

    fig.append(sg.RectElement(attrib = {
        'width': str(size[0]) + "px",
        'height': str(size[1]) + "px",
        'fill': str(backgroundColor[0])
    }))
...
def parseInput():
    """there are three keywords:
        ...
        -h: backgroundColor, hex value (including #) representing background of image
    ...
    backgroundColor = keywordDict['-h'] or u'#FFFFFF'
    ...
    return filenames,alpha,size,outDir,backgroundColor
